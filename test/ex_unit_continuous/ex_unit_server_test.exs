defmodule ExUnitContinuous.ExUnitServerASyncTest do
  ...
  test "_async_modules/0 allows to retrieve the list of async_modules on the server" do
    assert ExUnitServer._async_modules() == []  # empty as async tests are taken for a run just after being added when test suite is already started
  end
...
defmodule ExUnitContinuous.ExUnitServerTest do
  ...
  test "add_async_module/1 adds an async module, or exit cleanly" do
    on_exit(fn ->
      ExUnitServer._async_modules(fn sm -> sm |> List.delete(ExUnitContinuous.ExUnitServerASyncTest) end)
    end)
    # TODO : why isnt it "already running" here ?
    # Sometime we get exception from ExUnit.Server, but we can't prevent it ?
    assert :ok == ExUnitContinuous.ExUnitServer.add_async_module(
